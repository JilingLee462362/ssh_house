...
import com.ssh.house.dao.BaseDao;
/**
 * 通用的DAO的实现类
 * @author jt
 *
...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public Integer findCount(DetachedCriteria detachedCriteria) {
		...
		List<Long> list = (List<Long>) this.hibernateTemplate.findByCriteria(detachedCriteria);
		if(list.size() > 0){
	...
	public List<T> findByPage(DetachedCriteria detachedCriteria, Integer begin, Integer pageSize) {
		...
		return (List<T>) this.hibernateTemplate.findByCriteria(detachedCriteria, begin, pageSize);
	}
