...
<script type="text/javascript">
	$(document).ready(function(){
		/** 新增   **/
	    $("#addBtn").fancybox({
	    	'href'  : 'streetnumber/findOne.action',
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	        ...
	        'onClosed' : function() { 
	        	window.location.href = 'streetnumber/find.action';
	        }
	    });
	    ...
	    $("#importBtn").fancybox({
	        ...
	        'showCloseButton' : false,
	        'onClosed' : function() { 
	        	window.location.href = 'streetnumber_list.jsp';
	        }
	    });
	...
	/** 模糊查询来电用户  **/
	function search(){
		$("#submitForm").attr("action", "streetnumber_list.jsp?page=" + 1).submit();
	}
	...
	function del(fyID){
		...
		if(confirm("您确定要删除吗？")){
            ...
            //几个参数需要注意一下   4   200
                ...
                url: "streetnumber/delete.action?id=" +fyID,//url
...
// 				return;
		...
		if(allIDCheck.length>0) {
			...
			if(confirm("您确定要批量删除这些记录吗？")){
				// 提交form
				$("#submitForm").attr("action", "streetnumber/delete.action").submit();
			}
		}
	}
	...
	function jumpNormalPage(page){
		$("#submitForm").attr("action", "streetnumber/find.action?pageNum=" + page).submit();
	}
	...
	function jumpInputPage(totalPage){
		...
		if($("#jumpNumTxt").val() != ''){
			...
			$("#submitForm").attr("action", "streetnumber_list.jsp?page=" + pageNum).submit();
		}else{
			// “跳转页数”为空
			art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'请输入合适的页数，\n自动为您跳到首页', ok:true,});
			$("#submitForm").attr("action", "streetnumber_list.jsp?page=" + 1).submit();
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${StreetnumberList}" var="s">
							...
							<tr>
								...
								<td>${s.xiaoqu.name}</td>
								<td>
									<a href="streetnumber/findOne.action?id=${s.id}" class="edit">编辑</a> 
